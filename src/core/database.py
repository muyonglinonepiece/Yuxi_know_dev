...
class DataBaseManager:
    ...
    def add_qa_file(self, db_id, files, params=None):
        ...
        for new_file in new_files:
            ...
            try:
                ...
                self.knowledge_base.add_documents(
                    docs = [node.strip() for node in nodes if len(node.strip()) > 2],
                ...
                logger.error(f"Failed to add qa documents to collection {db.metaname}, {e}")
                idx = self.get_idx_by_fileid(db, file_id)
