...
class OpenAIBase():
    ...
    def predict(self, message, stream=False):
        if isinstance(message, str):
            if "中文" in message:
                messages = [{"role": "user", "content": "你是一个有用的助手，必须使用中文思考和回复。用户提问:"+message}]
            else:
                messages = [{"role": "user", "content":message}]
        else:
            messages = message
            if "中文" not in message[-1]["content"]:
                messages[-1]["content"] = "你是一个有用的助手，必须使用中文思考和回复。用户提问:"+message[-1]["content"]
